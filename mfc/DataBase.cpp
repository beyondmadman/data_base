...
CDataBaseEngine::CDataBaseEngine(void)
{
	...
	CDataBaseEngineSinkArray.RemoveAll();
...
bool  CDataBaseEngine::SetDataBaseEngineSink(IUnknownEx * pIUnknownExs[], WORD wSinkCount)
{
	...
	for (WORD i = 0; i < wSinkCount; i++)
	{
		...
		CDataBaseEngineSinkArray.Add(pIDataBaseEngineSink);
	}
	return true;
}
//启动服务
bool CDataBaseEngine::StartService()
{
	...
	//外挂接口
	if (/*m_pIDataBaseEngineSinks.empty()*/CDataBaseEngineSinkArray.IsEmpty())
	{
	...
	//启动外挂
	for (WORD i = 0; i < CDataBaseEngineSinkArray.GetCount(); i++)
	{
		if (CDataBaseEngineSinkArray[i]->OnDataBaseEngineStart(QUERY_ME_INTERFACE(IUnknownEx)) == false)
		{
...
bool CDataBaseEngine::ConcludeService()
{
	...
	for (WORD i = 0; i < CDataBaseEngineSinkArray.GetCount(); i++)
	{
		if (CDataBaseEngineSinkArray[i] != NULL)
		{
			CDataBaseEngineSinkArray[i]->OnDataBaseEngineConclude(QUERY_ME_INTERFACE(IUnknownEx));
		}
	}
	CDataBaseEngineSinkArray.RemoveAll();
...
void CDataBaseEngine::OnQueueServiceSink(WORD wIdentifier, void * pBuffer, WORD wDataSize)
{
	...
	switch (wIdentifier)
	{
		case EVENT_DATABASE:
		{
			...
			for (WORD i = 0; i < CDataBaseEngineSinkArray.GetCount(); i++)
			{
				ASSERT(CDataBaseEngineSinkArray[i] != NULL);
				CDataBaseEngineSinkArray[i]->OnDataBaseEngineRequest(pDataBaseEvent->wRequestID, pDataBaseEvent->dwContextID, pDataBaseEvent + 1, wHandleBuffer);
			}
		...
		case EVENT_TIMER://时间事件
		{
			...
			{
				ASSERT(CDataBaseEngineSinkArray[i] != NULL);
				CDataBaseEngineSinkArray[i]->OnDataBaseEngineTimer(pDataBaseEvent->dwTimerID, pDataBaseEvent->dwBindParameter);
			}
			return;
		}
		case EVENT_CONTROL://控制事件
		{
			...
			{
				ASSERT(CDataBaseEngineSinkArray[i] != NULL);
				CDataBaseEngineSinkArray[i]->OnDataBaseEngineControl(pDataBaseEvent->wControlID, pDataBaseEvent + 1, wHandleBuffer);
			}
			return;
		}
		default:
